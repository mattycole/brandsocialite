...
class BrandForm extends Component { 
    constructor(props) {
      ...
      componentWillMount() {
        let formRef = firebaseConf.database().ref('brands').orderByKey().limitToLast(6);
        formRef.on('child_added', snapshot => {
          const { name, email, city, phone, bartender_exp, message } = snapshot.val();
